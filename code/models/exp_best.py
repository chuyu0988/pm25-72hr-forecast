"""
BEST MODEL  (single model, seed 42, m=64)
=========================================
Best configuration found:
  INPUT (all 5 vars: PM2.5, WIND_U, WIND_V, RH, TEMP): raw observation where
      available, FPCA only fills missing cells.  + station lat/lon + station embedding(32).
  TEMPORAL: month sin/cos ONLY (2-d).  Weekday is dropped -- verified across seeds
      42/123/777 to consistently lower RMSE (no overlap with the month+weekday model).
  TRAIN TARGET PM2.5: raw where available, FPCA fills missing.  TEST TARGET: pure raw.

Result (seed 42): NCU-style RMSE = 7.2931, MAE = 5.2463
  (vs month+weekday baseline, same seed 42: 7.3250 / 5.2707).
Note: lat/lon is KEPT -- dropping it was verified to slightly HURT across seeds.
"""
...
# ============================================================
# Temporal: month sin/cos ONLY (weekday dropped -- verified better across seeds)
# ============================================================
...
print(f"\n{'='*60}\nRESULT (BEST model — exp_best, month-only temporal)\n{'='*60}")
...
print(f"  vs month+weekday baseline (same seed 42): 7.3250 / MAE 5.2707")
print(f"  vs SOTA target (uses CMAQ): 6.88")
print("\nDone!")
